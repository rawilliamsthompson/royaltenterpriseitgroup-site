...
function Insights(){return <><PageHero k="Insights" t="Clear guidance for modern technology leadership."/><section className="section"><div className="grid">{[['Identity & Access','Why individual accounts and MFA matter'],['Technology Governance','When growing businesses need Fractional CIO leadership'],['Microsoft 365','Turning Microsoft 365 into a governed platform']].map(x=><article className="card" key={x[1]}><small>{x[0]}</small><h3>{x[1]}</h3><p>Executive guidance designed to connect technology decisions with security, accountability, and business outcomes.</p></article>)}</div></section></>}
